"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.all = void 0;
const abi_1 = require("./abi");
const multicall_1 = require("./abi/multicall");
const ethers_1 = require("ethers");
const lodash_1 = require("lodash");
const service_1 = require("../service");
async function all(calls, multicallAddress, provider) {
    const multicall = new ethers_1.Contract(multicallAddress, multicall_1.multicallAbi, provider);
    const callRequests = calls.map((call) => {
        const callData = abi_1.Abi.encode(call.name, call.inputs, call.params);
        return {
            target: call.contract.address,
            callData
        };
    });
    let callRequestsChuck = (0, lodash_1.chunk)(callRequests, 255);
    try {
        let response = [];
        for (const callChuck of callRequestsChuck) {
            let result = await multicall.tryAggregate(false, callChuck);
            response.push(...result);
        }
        const callCount = calls.length;
        const callResult = [];
        for (let i = 0; i < callCount; i++) {
            const outputs = calls[i].outputs;
            const result = response[i];
            if (result.success) {
                const params = abi_1.Abi.decode(outputs, result.returnData);
                callResult.push(params);
            }
            else {
                callResult.push(undefined);
            }
        }
        return callResult;
    }
    catch (e) {
        service_1.Trace.error("mulcall call error", e);
        throw e;
    }
}
exports.all = all;
