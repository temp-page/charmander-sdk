...
let TokenMangerApi = class TokenMangerApi {
        ...
        if (search === "") {
            ...
            if ((0, ethers_1.isAddress)(search)) {
                try {
                    ...
                    let erc20Infos = await addressInfo.readonlyConnectInfo().erc20().getErc20Info(search);
                    let searchRpcTokens = erc20Infos
                        .filter(it => !(0, tool_1.isNullOrBlank)(it.name) && !(0, tool_1.isNullOrBlank)(it.symbol) && it.decimals > 0)
                        .map(it => {
                        return new tool_2.Token(addressInfo.chainId, it.address, it.decimals, it.symbol, it.name, `https://agni.finance/static/${it.symbol}.png`);
                    })
            ...
            let tokensByTokenList = Array.from(await this.tokenList())
                .flatMap(it => it.tokenList.tokens.map(it => tool_2.Token.fromSerialized(it)))
                .filter(it => {
                return it.address.toLowerCase().indexOf(search) > -1 || it.symbol.toLowerCase().indexOf(search) > -1 || it.name.toLowerCase().indexOf(search) > -1;
            })
                .map(it => this.mapToTokenAdd(it, storageToken));
    ...
    systemTokens() {
        return tokens_json_1.default.filter(t => t.chainId === this.baseApi.address().chainId)
            .map(it => {
            return new tool_2.Token(this.baseApi.address().chainId, it.address, it.decimals, it.symbol, it.name, it.logoURI);
        });
    }
