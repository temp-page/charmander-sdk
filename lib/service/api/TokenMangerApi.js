...
let TokenMangerApi = class TokenMangerApi {
    ...
    async tokenManager(searchStr = '') {
        ...
        const mapToTokenAdd = (token, storageToken) => {
        ...
        if (search === "") {
            return {
                searchTokens: [],
                customTokens: storageToken.map(mapToTokenManager.call(this))
            };
        }
        else {
            let searchTokens = [];
            if ((0, ethers_1.isAddress)(search)) {
                try {
                    let erc20Infos = await this.baseApi.address().readonlyConnectInfo().erc20().getErc20Info(search);
                    let searchRpcTokens = erc20Infos
                        .filter(it => !(0, tool_1.isNullOrBlank)(it.name) && !(0, tool_1.isNullOrBlank)(it.symbol) && it.decimals > 0)
                        .map(it => {
                        return new vo_1.Token(it.address, it.decimals, it.symbol, it.name, `https://agni.finance/static/${it.symbol}.png`);
                    })
                        .map(it => mapToTokenAdd(it, storageToken));
                    searchTokens.push(...searchRpcTokens);
                }
                catch (e) {
                    tool_1.Trace.error("searchTokens error ignore", e, search);
                }
            }
            let tokensByTokenList = Array.from(await this.tokenList())
                .flatMap(it => it.tokenList.tokens.map(it => vo_1.Token.fromSerialized(it)))
                .filter(it => {
    ...
    systemTokens() {
        return tokens_json_1.default.filter(t => t.chainId === this.baseApi.address().chainId)
            .map(it => {
            return new vo_1.Token(it.address, it.decimals, it.symbol, it.name, it.logoURI);
        });
    }
