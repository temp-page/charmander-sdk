...
let PoolV3Api = PoolV3Api_1 = class PoolV3Api {
    ...
    async myLiquidityList(connectInfo) {
        ...
        let pools = await this.getPool(positions.map(it => {
            ...
            return {
                token0, token1, feeAmount: parseInt(it.position.fee)
            };
        }));
        const allPosition = positions.map((it, index) => {
            ...
            let rank1 = sortRank[a.state];
            let rank2 = sortRank[b.state];
            if (rank1 !== rank2) {
                return rank1 - rank2;
            }
    ...
    initLiquidityData(liquidityDetails, tokenId, token0, token1, pool, position) {
        ...
        liquidityDetails.feeAmount = parseInt(position.fee);
        let price = pool.priceOf(token1).toFixed();
        liquidityDetails.minPrice = (0, Common_1.tickToPriceString)(token1, token0, liquidityDetails.feeAmount, parseInt(position.tickUpper));
        liquidityDetails.maxPrice = (0, Common_1.tickToPriceString)(token1, token0, liquidityDetails.feeAmount, parseInt(position.tickLower));
        ...
        if (new bignumber_js_1.default(position.liquidity).comparedTo("0") > 0) {
            if (pool.tickCurrent < parseInt(position.tickLower) || pool.tickCurrent >= parseInt(position.tickUpper)) {
    ...
    async myLiquidityByTokenId(connectInfo, tokenId) {
        ...
        liquidityInfo.apr = parseFloat(liquidityInfo.liquidityUSD) <= 0 ? "0" : liquidityHistory.filter(it => it.type == 'collect_fee')
        ...
        liquidityInfo.collectFee = async (connect, involvesMNT) => {
            let transactionEvent = await connect.create(abi_1.NonfungiblePositionManager, this.baseApi.address().nonfungiblePositionManager)
        ...
        liquidityInfo.removeLiquidity = async (connect, rate, involvesMNT, allowedSlippage, deadline) => {
            let preRemoveLiquidity = liquidityInfo.preRemoveLiquidity(rate);
            let positionDetail = new v3_1.Position({
                pool,
                liquidity: new bignumber_js_1.default(position.liquidity).multipliedBy(rate).toFixed(0, bignumber_js_1.default.ROUND_DOWN),
                tickLower: parseInt(position.tickLower),
        ...
        liquidityInfo.preAddLiquidity = (inputToken, inputAmount) => {
            ...
            let outAmount = this.outputTokenAmount(pool, inputToken, inputAmount, parseInt(position.tickLower), parseInt(position.tickUpper));
    ...
    async addLiquidity(token0, token1, account) {
        ...
        let [poolDatas, feeTierDistribution] = await Promise.all([
        ...
        addLiquidityV3Info.addLiquidity = async (connect, allowedSlippage, deadline) => {
            ...
            let slippageTolerance = new tool_1.Percent(BigInt(new bignumber_js_1.default(allowedSlippage).multipliedBy(10000).toFixed(0, bignumber_js_1.default.ROUND_DOWN)), 10000n);
            let deadlineReal = parseInt(String(new Date().getTime() / 1000)) + parseInt(deadline.toString());
            let transactionEvent = await nonfungiblePositionManager.addLiquidity(position, null, addLiquidityV3Info.first, slippageTolerance, deadlineReal);
