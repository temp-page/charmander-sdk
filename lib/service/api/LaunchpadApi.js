...
const InsurancePoolAbi_1 = require("../abi/InsurancePoolAbi");
...
let LaunchpadApi = class LaunchpadApi {
    ...
    async getUserStakeInfoByGql(user, token) {
        const res = await this.baseApi.launchpadGraph(gql_1.UserStakeInfosGQL, {
    ...
    async staking(account = "") {
        const userAddress = account ? account : tool_1.INVALID_ADDRESS;
        let tokenAddress = this.baseApi.address().launchpadStakeToken;
        let launchpadStakePoolAddress = this.baseApi.address().launchpadStakePool;
        let stakingPoolAbi = this.baseApi.connectInfo().create(abi_1.StakingPoolAbi);
        let [stakeInfos, batchGetTokens, [{ getScoreByTier, getUserScore }]] = await Promise.all([
        ...
        let stakeTokenInfo = batchGetTokens[tokenAddress];
        let { [stakeTokenInfo.address]: balanceAndAllowances } = await this.baseApi.connectInfo().erc20().batchGetBalanceAndAllowance(userAddress, launchpadStakePoolAddress, [stakeTokenInfo]);
        ...
        stakeDetail.stake = async (connect, amount) => {
            let realAmount = new bignumber_js_1.default(amount).multipliedBy(10 ** stakeDetail.token.decimals).toFixed(0, bignumber_js_1.default.ROUND_DOWN);
            return connect.create(abi_1.StakingPoolAbi).stake(stakeDetail.token.address, realAmount);
        };
        stakeDetail.unStake = async (connect) => {
            const ids = unStakeInfo.map(it => it.id).slice(0, 10);
            if (ids.length === 0) {
                throw new BasicException_1.BasicException("No unstake tokenAddress");
            }
            return connect.create(abi_1.StakingPoolAbi).unstake(ids);
        };
        return stakeDetail;
    }
    async getTokenPriceHistory(token) {
        ...
        let { tokenDayDatas } = await this.baseApi.exchangeGraph(gql_1.QueryTokenATHPriceHistory, {
            token: token.toLowerCase()
        });
        const tokenPrice = {
            ath: "0",
        ...
        tokenPrice.last = lodash_1.default.get(tokenDayDatas, "[0].priceUSD", "0");
    ...
    async getPools() {
        const [res, launchpadInfos] = await Promise.all([
            this.baseApi.launchpadGraph(gql_1.QueryIdoPoolInfosGQL, {}),
            this.fetchPools()
        ]);
        const allProject = res.idoPools
            ...
            idoPool.roi = '0';
            idoPool.presaleAndEnrollStartTime = parseInt(item.presaleAndEnrollStartTime, 10);
            idoPool.soldOut = parseInt(item.claimStartTime, 10) * 1000 < Date.now();
            return idoPool;
        });
        ...
        idoPoolStatistics.totalParticipants = lodash_1.default.get(res, 'idoPoolStatistics.totalParticipants', '0');
        ...
        const poolInfo = {
            ...
            comingProjects: Array.from(allProject)
                .filter((it) => it.presaleAndEnrollStartTime < Date.now() / 1000)
                .sort((a, b) => {
    ...
    async getUserDepositedLogsByPool(user, pool) {
        let res = await this.baseApi.launchpadGraph(gql_1.QueryIDOUserDepositedLogsByPool, {
            user: user.toLowerCase(),
    ...
    async getPoolInfoByGql(id) {
        const [res, launchpadInfos] = await Promise.all([
            this.baseApi.launchpadGraph(gql_1.QueryIDOPoolInfo, {
                id,
            }),
            this.fetchPools()
        ]);
    ...
    async poolDetail(pool, account = "") {
        ...
        const insurancePool = this.baseApi.connectInfo().create(InsurancePoolAbi_1.InsurancePoolAbi);
        const idoPool = this.baseApi.connectInfo().create(abi_1.IdoPoolAbi, pool);
        const stakingPool = this.baseApi.connectInfo().create(abi_1.StakingPoolAbi);
        const multiCall = this.baseApi.connectInfo().multiCall();
        const tokenIns = this.baseApi.connectInfo().create(abi_1.ERC20, poolInfo.sellingTokenInfo.id);
        let [insurancePoolData, idoPoolData, { getCurrentBlockTimestamp }, { getUserTier }, { totalSupply },] = await multiCall
            .call({
        ...
        const unixTime = parseInt(getCurrentBlockTimestamp);
        const depositInfo = new vo_1.IDODepositInfo();
        const raisingTokenDiv = 10 ** parseInt(poolInfo.raisingTokenInfo.decimals, 10);
        const sellingTokenDiv = 10 ** parseInt(poolInfo.sellingTokenInfo.decimals, 10);
        let { [poolInfo.raisingTokenInfo.id]: token } = await this.baseApi.address().getApi().tokenMangerApi().batchGetTokens([poolInfo.raisingTokenInfo.id]);
        ...
        depositInfo.whiteList = {
            type: "whiteList",
            canDeposit: parseInt(poolInfo.presaleAndEnrollStartTime, 10) <= unixTime && parseInt(poolInfo.presaleAndEnrollEndTime, 10) >= unixTime && (!idoPoolData.presaleDeposited),
            // 是否质押
        ...
        depositInfo.publicSale = {
            type: "publicSale",
            canDeposit: parseInt(poolInfo.publicSaleDepositStartTime, 10) <= unixTime && parseInt(poolInfo.publicSaleDepositEndTime, 10) >= unixTime && (!idoPoolData.publicSaleDeposited),
        ...
        depositInfo.canEnroll = (!idoPoolData.isEnrolled) && depositInfo.checkUserTier && parseInt(poolInfo.presaleAndEnrollStartTime, 10) <= unixTime && parseInt(poolInfo.presaleAndEnrollEndTime, 10) >= unixTime;
        ...
        if (depositInfo.publicSale.canDeposit) {
            ...
            depositInfo.depositMaxInput = new bignumber_js_1.default(idoPoolData.getPublicSaleQuota)
                ...
                .toFixed(parseInt(poolInfo.raisingTokenInfo.decimals, 10), bignumber_js_1.default.ROUND_DOWN);
        ...
        depositInfo.claimLoss = (connect, insuranceId) => {
            return connect.create(InsurancePoolAbi_1.InsurancePoolAbi).claimLoss(insuranceId);
        };
        depositInfo.enroll = async (connect) => {
            return connect.create(abi_1.IdoPoolAbi, pool).enroll();
        };
        depositInfo.claim = async (connect) => {
            return connect.create(abi_1.IdoPoolAbi, pool).claim();
        };
        ...
        depositInfo.calculateQuote = (depositAmount) => {
            ...
            if (depositInfo.currentDeposit.type === 'whiteList') {
                return depositAmountBN.dividedBy(poolInfo.presalePrice).toFixed(parseInt(poolInfo.sellingTokenInfo.decimals), bignumber_js_1.default.ROUND_DOWN);
            }
            return depositAmountBN.dividedBy(poolInfo.publicSalePrice).toFixed(parseInt(poolInfo.sellingTokenInfo.decimals), bignumber_js_1.default.ROUND_DOWN);
        };
        depositInfo.deposit = async (connect, buyInsurance, depositAmount, extraDeposit) => {
            ...
            if (depositInfo.currentDeposit.type === 'whiteList') {
                ...
                return connect.create(abi_1.IdoPoolAbi, pool).presaleDeposit(buyQuota, buyInsurance);
            }
            ...
            return connect.create(abi_1.IdoPoolAbi, pool).publicSaleDeposit(buyInsurance, buyQuota, extraDepositReal);
        };
        if (new bignumber_js_1.default(poolInfo.claimStartTime).comparedTo(unixTime) < 0) {
            ...
            depositInfo.whiteList.countdownEndTime = 0;
            depositInfo.publicSale.countdownEndTime = 0;
            if (parseInt(poolInfo.presaleAndEnrollStartTime, 10) <= unixTime && parseInt(poolInfo.presaleAndEnrollEndTime, 10) >= unixTime) {
                // 白名单时间段，没质押，有可用额度
                if (!idoPoolData.presaleDeposited && parseFloat(depositInfo.whiteList.maxDepositAmount) > 0) {
                    depositInfo.whiteList.depositStatus = 'enable';
                }
            }
            else if (parseInt(poolInfo.publicSaleDepositStartTime, 10) <= unixTime && parseInt(poolInfo.publicSaleDepositEndTime, 10) >= unixTime) {
                // 存在(白名单和公售)质押的情况 或者 可质押额度>0
                if (parseFloat(depositInfo.publicSale.depositAmount) > 0 || parseFloat(depositInfo.whiteList.depositAmount) > 0
                    || parseFloat(depositInfo.publicSale.maxDepositAmount) > parseFloat(depositInfo.publicSale.depositAmount)) {
            ...
            if (parseInt(poolInfo.claimStartTime, 10) > unixTime) {
                depositInfo.whiteList.countdownEndTime = parseInt(poolInfo.claimStartTime, 10);
                depositInfo.publicSale.countdownEndTime = parseInt(poolInfo.claimStartTime, 10);
            }
            if (parseInt(poolInfo.publicSaleDepositEndTime, 10) > unixTime) {
                depositInfo.publicSale.countdownEndTime = parseInt(poolInfo.publicSaleDepositEndTime, 10);
            }
            if (parseInt(poolInfo.publicSaleDepositStartTime, 10) > unixTime) {
                depositInfo.publicSale.countdownEndTime = parseInt(poolInfo.publicSaleDepositStartTime, 10);
            }
            if (parseInt(poolInfo.presaleAndEnrollEndTime, 10) > unixTime) {
                depositInfo.whiteList.countdownEndTime = parseInt(poolInfo.presaleAndEnrollEndTime, 10);
            }
            if (parseInt(poolInfo.presaleAndEnrollStartTime, 10) > unixTime) {
                depositInfo.whiteList.countdownEndTime = parseInt(poolInfo.presaleAndEnrollStartTime, 10);
            }
        }
        const idoByUser = idoPoolData.getUserIDO;
        if (depositInfo.claimStatus === "enable") {
            if (new bignumber_js_1.default(depositInfo.avgPrice).comparedTo("0") > 0 && new bignumber_js_1.default(depositInfo.needToPay).comparedTo("0") > 0) {
                let insuranceDetails = await multiCall.call(idoByUser.insuranceIds.map(it => {
                    return {
                ...
                // 检查是不是wait
                const time = lodash_1.default.get(launchpadInfo, "redemption_time", "0");
                // 如果未到时间
                let state = "disabled";
        ...
        // 总出售 - 白名单额度
        poolDetail.publicAllocation = bignumber_js_1.default.max(new bignumber_js_1.default(lodash_1.default.get(launchpadInfo, 'pool_size', '0')).minus(poolInfo.whiteListQuota), "0").toFixed();
        ...
        if (launchpadInfo) {
            poolDetail.poolSize = lodash_1.default.get(launchpadInfo, 'pool_size', '');
            poolDetail.initialMarketCap = lodash_1.default.get(launchpadInfo, 'initial_market_cap', '');
            poolDetail.FDV = lodash_1.default.get(launchpadInfo, 'fdv', '');
            poolDetail.tags = lodash_1.default.get(launchpadInfo, 'selling_token_tag', '');
            poolDetail.whitelistStakingTierRequired = lodash_1.default.get(launchpadInfo, 'whitelist_staking_tier_required', '');
            poolDetail.whitelistRegistrationRequired = lodash_1.default.get(launchpadInfo, 'whitelist_registration_required', '');
            poolDetail.whitelistDistribution = lodash_1.default.get(launchpadInfo, 'whitelist_distribution', '');
            poolDetail.publicStakingTierRequired = lodash_1.default.get(launchpadInfo, 'public_registration_required', '');
            poolDetail.publicRegistrationRequired = lodash_1.default.get(launchpadInfo, 'public_registration_required', '');
            poolDetail.publicDistribution = lodash_1.default.get(launchpadInfo, 'public_distribution', '');
            poolDetail.introduction = lodash_1.default.get(launchpadInfo, 'introduction', '');
            poolDetail.shares = lodash_1.default.get(launchpadInfo, 'shares', []).filter(it => it.icon);
        }
        else {
            poolDetail.poolSize = "";
