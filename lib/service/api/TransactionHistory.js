...
class TransactionHistory {
    ...
    initUpdateTransaction(connectInfo, start) {
        if (start) {
            ...
            TransactionHistory.connect = null;
            TransactionHistory.start = start;
    ...
    async updateTransaction(connectInfo) {
        let transactions = this.storageHistories(connectInfo);
        let storageRecentTransactions = transactions.filter(it => it.status === "pending");
        for (let storageRecentTransaction of storageRecentTransactions) {
            try {
    ...
    histories(connectInfo) {
        let storageRecentTransactions = this.storageHistories(connectInfo);
        return Array.from(storageRecentTransactions).reverse().map(it => {
            let chainName = connectInfo.addressInfo.chainName;
            let title = "";
            switch (it.type) {
                case "remove":
