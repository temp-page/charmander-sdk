"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Token = exports.BalanceAndAllowance = exports.Balance = exports.DEFAULT_ICON = exports.ETH_ADDRESS = void 0;
const Constant_1 = require("../../Constant");
const bignumber_js_1 = __importDefault(require("bignumber.js"));
exports.ETH_ADDRESS = 'MNT';
exports.DEFAULT_ICON = "data:image/svg+xml;base64,";
class Balance {
    constructor(token, user, balance) {
        this.token = token;
        this.user = user;
        this.balance = balance;
    }
}
exports.Balance = Balance;
class BalanceAndAllowance extends Balance {
    constructor(token, user, balance, allowance, spender) {
        super(token, user, balance);
        this.allowance = allowance;
        this.spender = spender;
    }
    showApprove(inputAmount) {
        if (this.token.address === exports.ETH_ADDRESS) {
            return false;
        }
        return new bignumber_js_1.default(inputAmount).comparedTo(this.allowance) > 0;
    }
    approve(connectInfo) {
        return connectInfo.erc20().approve(this.spender, this.token.erc20Address());
    }
}
exports.BalanceAndAllowance = BalanceAndAllowance;
class Token {
    constructor(address, decimals, symbol, name, logoURI) {
        this.address = address;
        this.decimals = decimals;
        this.symbol = symbol;
        this.name = name;
        this.logoURI = logoURI;
    }
    static fromSerialized(serialized) {
        return new Token(serialized.address, serialized.decimals, serialized.symbol, serialized.name, serialized.logoURI);
    }
    equals(other) {
        return other && other.address && this.address === other.address;
    }
    sortsBefore(other) {
        return this.erc20Address().toLowerCase() < other.erc20Address().toLowerCase();
    }
    erc20Address() {
        return this.address === exports.ETH_ADDRESS ? (0, Constant_1.getCurrentAddressInfo)().WMNT : this.address;
    }
    get serialize() {
        return {
            address: this.address,
            chainId: this.chainId,
            decimals: this.decimals,
            symbol: this.symbol,
            name: this.name,
            logoURI: this.logoURI
        };
    }
    iconUrl() {
        return this.logoURI ? this.logoURI : exports.DEFAULT_ICON;
    }
    scanUrl() {
        return this.address === exports.ETH_ADDRESS ? "" : (0, Constant_1.getCurrentAddressInfo)().getEtherscanAddress(this.address);
    }
}
exports.Token = Token;
