...
export declare class LiquidityInfo {
    ...
    removeLiquidity: (connect: ConnectInfo, rate: string, involvesMNT: boolean, allowedSlippage: string, deadline: string) => Promise<TransactionEvent>;
    preAddLiquidity: (inputToken: Token, inputAmount: string) => {
        amount0: string;
        amount1: string;
    };
    addLiquidity: (connect: ConnectInfo, amount0: string, amount1: string, allowedSlippage: string, deadline: string) => Promise<TransactionEvent>;
}
export interface LiquidityHistory {
    time: number;
    action: 'add' | 'remove' | 'collect_fee';
    token0Amount: string;
    token1Amount: string;
}
