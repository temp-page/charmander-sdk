...
let TransactionService = class TransactionService extends BaseService_1.BaseService {
    ...
    async checkTransactionError(txId) {
        ...
        while (count >= 0) {
            ...
            tool_1.Trace.print('checkTransactionError', res);
            if (res?.status != null && res.hash.toLowerCase() === txId.toLowerCase()) {
                if (res.status) {
                    return res;
                }
                else {
    ...
    async sendContractTransaction(contract, method, args = [], config = {}) {
        const currentChain = (0, Constant_1.getCurrentAddressInfo)().chainId;
        const chainId = parseInt((await this.connectInfo.provider.getNetwork()).chainId.toString());
            ...
            while (recursiveErr) {
                reason =
                    lodash_1.default.get(recursiveErr, 'reason') ||
                        ...
                        lodash_1.default.get(recursiveErr, 'info.error.message') ||
                        reason;
                recursiveErr = lodash_1.default.get(recursiveErr, 'error') || lodash_1.default.get(recursiveErr, 'data.originalError') || lodash_1.default.get(recursiveErr, 'info');
            }
        }
