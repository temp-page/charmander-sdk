...
function tickToPriceString(token0, token1, feeAmount, tick) {
    let min = (0, utils_1.nearestUsableTick)(utils_1.TickMath.MIN_TICK, vo_1.TICK_SPACINGS[feeAmount]);
    let max = (0, utils_1.nearestUsableTick)(utils_1.TickMath.MAX_TICK, vo_1.TICK_SPACINGS[feeAmount]);
    if (tick === min) {
        return "0";
    }
    if (tick === max) {
