...
let Erc20Service = class Erc20Service extends BaseService_1.BaseService {
    ...
    async batchGetBalanceInfo(user, tokens) {
        ...
        result.forEach((it, index) => {
            data[it.address] = new vo_1.Balance(tokens[index], user, new bignumber_js_1.default(it.balance).div(it.decimals).toFixed());
        });
        return data;
    }
    /**
    ...
    async batchGetBalanceAndAllowance(user, spender, tokens) {
        ...
        const data = {};
        result.forEach((it, index) => {
            data[it.address] = new vo_1.BalanceAndAllowance(tokens[index], user, new bignumber_js_1.default(it.balance).div(it.decimals).toFixed(), new bignumber_js_1.default(it.allowance).div(it.decimals).toFixed(), spender);
        });
        return data;
    }
};
