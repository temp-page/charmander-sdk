...
function getCreate2Address(from_, salt_, initCodeHash) {
    return ethers6_1.ethers.getCreate2Address(from_, salt_, initCodeHash);
}
const computePairAddress = ({ factoryAddress, tokenA, tokenB, }) => {
    const [token0, token1] = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA]; // does safety checks
    const key = composeKey(token0, token1);
    if (PAIR_ADDRESS_CACHE[key] === undefined) {
        PAIR_ADDRESS_CACHE[key] = getCreate2Address(factoryAddress, (0, ethers6_1.solidityPackedKeccak256)(['address', 'address'], [token0.address, token1.address]), (0, Constant_1.getCurrentAddressInfo)().v2InitCodeHash);
    }
