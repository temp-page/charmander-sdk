...
function tradeComparator(a, b) {
  (0, _Common.invariant)(a.inputAmount.currency.equals(b.inputAmount.currency), "INPUT_CURRENCY");
  (0, _Common.invariant)(a.outputAmount.currency.equals(b.outputAmount.currency), "OUTPUT_CURRENCY");
...
class Trade {
   ...
   */
  get route() {
    (0, _Common.invariant)(this.swaps.length === 1, "MULTIPLE_ROUTES");
   ...
   */
  get executionPrice() {
    return this._executionPrice ?? (this._executionPrice = new _fractions.Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.quotient, this.outputAmount.quotient));
  }
  /**
  ...
  static async exactIn(route, amountIn) {
    return Trade.fromRoute(route, amountIn, _constants.TradeType.EXACT_INPUT);
  }
  /**
  ...
  static async exactOut(route, amountOut) {
    return Trade.fromRoute(route, amountOut, _constants.TradeType.EXACT_OUTPUT);
  }
  /**
  ...
  static async fromRoute(route, amount, tradeType) {
    ...
    let inputAmount;
    let outputAmount;
    if (tradeType === _constants.TradeType.EXACT_INPUT) {
      (0, _Common.invariant)(amount.currency.equals(route.input), "INPUT");
      ...
      (0, _Common.invariant)(amount.currency.equals(route.output), "OUTPUT");
  ...
  static async fromRoutes(routes, tradeType) {
    ...
    for (const {
      ...
      if (tradeType === _constants.TradeType.EXACT_INPUT) {
        (0, _Common.invariant)(amount.currency.equals(route.input), "INPUT");
        inputAmount = _fractions.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
        amounts[0] = _fractions.CurrencyAmount.fromFractionalAmount(route.input.wrapped, amount.numerator, amount.denominator);
        for (let i = 0; i < route.tokenPath.length - 1; i++) {
          const pool = route.pools[i];
          const [outputAmount2] = await pool.getOutputAmount(amounts[i]);
          amounts[i + 1] = outputAmount2;
        ...
        (0, _Common.invariant)(amount.currency.equals(route.output), "OUTPUT");
        outputAmount = _fractions.CurrencyAmount.fromFractionalAmount(route.output, amount.numerator, amount.denominator);
        amounts[amounts.length - 1] = _fractions.CurrencyAmount.fromFractionalAmount(route.output.wrapped, amount.numerator, amount.denominator);
        for (let i = route.tokenPath.length - 1; i > 0; i--) {
          const pool = route.pools[i - 1];
          const [inputAmount2] = await pool.getInputAmount(amounts[i]);
          amounts[i - 1] = inputAmount2;
  ...
  constructor({
    ...
    (0, _Common.invariant)(routes.every(({
      route
    }) => inputCurrency.wrapped.equals(route.input.wrapped)), "INPUT_CURRENCY_MATCH");
    (0, _Common.invariant)(routes.every(({
      route
    }) => outputCurrency.wrapped.equals(route.output.wrapped)), "OUTPUT_CURRENCY_MATCH");
    ...
    for (const {
      ...
      for (const pool of route.pools) poolAddressSet.add(_pool.Pool.getAddress(pool.token0, pool.token1, pool.fee));
    }
    (0, _Common.invariant)(numPools === poolAddressSet.size, "POOLS_DUPLICATED");
    this.swaps = routes;
  ...
  minimumAmountOut(slippageTolerance, amountOut = this.outputAmount) {
    (0, _Common.invariant)(!slippageTolerance.lessThan(_constants.ZERO), "SLIPPAGE_TOLERANCE");
    if (this.tradeType === _constants.TradeType.EXACT_OUTPUT) return amountOut;
    const slippageAdjustedAmountOut = new _fractions.Fraction(_constants.ONE).add(slippageTolerance).invert().multiply(amountOut.quotient).quotient;
  ...
  maximumAmountIn(slippageTolerance, amountIn = this.inputAmount) {
    (0, _Common.invariant)(!slippageTolerance.lessThan(_constants.ZERO), "SLIPPAGE_TOLERANCE");
    if (this.tradeType === _constants.TradeType.EXACT_INPUT) return amountIn;
    const slippageAdjustedAmountIn = new _fractions.Fraction(_constants.ONE).add(slippageTolerance).multiply(amountIn.quotient).quotient;
   ...
   * @returns The exact in trade
   */
  static async bestTradeExactIn(pools, currencyAmountIn, currencyOut, {
    maxNumResults = 3,
    maxHops = 3
  } = {}, currentPools = [], nextAmountIn = currencyAmountIn, bestTrades = []) {
    (0, _Common.invariant)(pools.length > 0, "POOLS");
    (0, _Common.invariant)(maxHops > 0, "MAX_HOPS");
    (0, _Common.invariant)(currencyAmountIn === nextAmountIn || currentPools.length > 0, "INVALID_RECURSION");
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      if (amountOut.currency.isToken && amountOut.currency.equals(tokenOut)) {
        (0, _utils.sortedInsert)(bestTrades, await Trade.fromRoute(new _route.Route([...currentPools, pool], currencyAmountIn.currency, currencyOut), currencyAmountIn, _constants.TradeType.EXACT_INPUT), maxNumResults, tradeComparator);
   ...
   * @returns The exact out trade
   */
  static async bestTradeExactOut(pools, currencyIn, currencyAmountOut, {
    maxNumResults = 3,
    maxHops = 3
  } = {}, currentPools = [], nextAmountOut = currencyAmountOut, bestTrades = []) {
    (0, _Common.invariant)(pools.length > 0, "POOLS");
    (0, _Common.invariant)(maxHops > 0, "MAX_HOPS");
    (0, _Common.invariant)(currencyAmountOut === nextAmountOut || currentPools.length > 0, "INVALID_RECURSION");
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      try {
        ...
        if (error.isInsufficientReservesError) continue;
        throw error;
      }
      if (amountIn.currency.equals(tokenIn)) {
        (0, _utils.sortedInsert)(bestTrades, await Trade.fromRoute(new _route.Route([pool, ...currentPools], currencyIn, currencyAmountOut.currency), currencyAmountOut, _constants.TradeType.EXACT_OUTPUT), maxNumResults, tradeComparator);
      } else if (maxHops > 1 && pools.length > 1) {
        const poolsExcludingThisPool = pools.slice(0, i).concat(pools.slice(i + 1, pools.length));
