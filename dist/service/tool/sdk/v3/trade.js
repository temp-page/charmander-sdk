...
function tradeComparator(a, b) {
    ...
    (0, Common_1.invariant)(a.inputAmount.currency.equals(b.inputAmount.currency), 'INPUT_CURRENCY');
    (0, Common_1.invariant)(a.outputAmount.currency.equals(b.outputAmount.currency), 'OUTPUT_CURRENCY');
...
class Trade {
     ...
     */
    get route() {
        (0, Common_1.invariant)(this.swaps.length === 1, 'MULTIPLE_ROUTES');
    ...
    get executionPrice() {
        return (this._executionPrice
            ?? (this._executionPrice = new fractions_1.Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.quotient, this.outputAmount.quotient)));
    }
    /**
    ...
    static async exactIn(route, amountIn) {
        return Trade.fromRoute(route, amountIn, constants_1.TradeType.EXACT_INPUT);
    }
    /**
    ...
    static async exactOut(route, amountOut) {
        return Trade.fromRoute(route, amountOut, constants_1.TradeType.EXACT_OUTPUT);
    }
    /**
    ...
    static async fromRoute(route, amount, tradeType) {
        ...
        if (tradeType === constants_1.TradeType.EXACT_INPUT) {
            (0, Common_1.invariant)(amount.currency.equals(route.input), 'INPUT');
            ...
            (0, Common_1.invariant)(amount.currency.equals(route.output), 'OUTPUT');
    ...
    static async fromRoutes(routes, tradeType) {
        ...
        for (const { route, amount } of routes) {
            ...
            if (tradeType === constants_1.TradeType.EXACT_INPUT) {
                (0, Common_1.invariant)(amount.currency.equals(route.input), 'INPUT');
                inputAmount = fractions_1.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
                amounts[0] = fractions_1.CurrencyAmount.fromFractionalAmount(route.input.wrapped, amount.numerator, amount.denominator);
                for (let i = 0; i < route.tokenPath.length - 1; i++) {
                    const pool = route.pools[i];
                    const [outputAmount] = await pool.getOutputAmount(amounts[i]);
                    amounts[i + 1] = outputAmount;
                ...
                (0, Common_1.invariant)(amount.currency.equals(route.output), 'OUTPUT');
                outputAmount = fractions_1.CurrencyAmount.fromFractionalAmount(route.output, amount.numerator, amount.denominator);
                amounts[amounts.length - 1] = fractions_1.CurrencyAmount.fromFractionalAmount(route.output.wrapped, amount.numerator, amount.denominator);
                for (let i = route.tokenPath.length - 1; i > 0; i--) {
                    const pool = route.pools[i - 1];
                    const [inputAmount] = await pool.getInputAmount(amounts[i]);
                    amounts[i - 1] = inputAmount;
    ...
    constructor({ routes, tradeType, }) {
        ...
        (0, Common_1.invariant)(routes.every(({ route }) => inputCurrency.wrapped.equals(route.input.wrapped)), 'INPUT_CURRENCY_MATCH');
        (0, Common_1.invariant)(routes.every(({ route }) => outputCurrency.wrapped.equals(route.output.wrapped)), 'OUTPUT_CURRENCY_MATCH');
        ...
        (0, Common_1.invariant)(numPools === poolAddressSet.size, 'POOLS_DUPLICATED');
        this.swaps = routes;
    ...
    minimumAmountOut(slippageTolerance, amountOut = this.outputAmount) {
        (0, Common_1.invariant)(!slippageTolerance.lessThan(constants_1.ZERO), 'SLIPPAGE_TOLERANCE');
    ...
    maximumAmountIn(slippageTolerance, amountIn = this.inputAmount) {
        ...
        if (this.tradeType === constants_1.TradeType.EXACT_INPUT)
            return amountIn;
        const slippageAdjustedAmountIn = new fractions_1.Fraction(constants_1.ONE).add(slippageTolerance).multiply(amountIn.quotient).quotient;
    ...
    currentPools = [], nextAmountIn = currencyAmountIn, bestTrades = []) {
        ...
        (0, Common_1.invariant)(maxHops > 0, 'MAX_HOPS');
        (0, Common_1.invariant)(currencyAmountIn === nextAmountIn || currentPools.length > 0, 'INVALID_RECURSION');
        ...
        for (let i = 0; i < pools.length; i++) {
            ...
            if (amountOut.currency.isToken && amountOut.currency.equals(tokenOut)) {
                (0, utils_1.sortedInsert)(bestTrades, await Trade.fromRoute(new route_1.Route([...currentPools, pool], currencyAmountIn.currency, currencyOut), currencyAmountIn, constants_1.TradeType.EXACT_INPUT), maxNumResults, tradeComparator);
            }
    ...
    currentPools = [], nextAmountOut = currencyAmountOut, bestTrades = []) {
        (0, Common_1.invariant)(pools.length > 0, 'POOLS');
        (0, Common_1.invariant)(maxHops > 0, 'MAX_HOPS');
        (0, Common_1.invariant)(currencyAmountOut === nextAmountOut || currentPools.length > 0, 'INVALID_RECURSION');
        ...
        for (let i = 0; i < pools.length; i++) {
            ...
            if (amountIn.currency.equals(tokenIn)) {
                (0, utils_1.sortedInsert)(bestTrades, await Trade.fromRoute(new route_1.Route([pool, ...currentPools], currencyIn, currencyAmountOut.currency), currencyAmountOut, constants_1.TradeType.EXACT_OUTPUT), maxNumResults, tradeComparator);
            }
            else if (maxHops > 1 && pools.length > 1) {
                const poolsExcludingThisPool = pools.slice(0, i).concat(pools.slice(i + 1, pools.length));
