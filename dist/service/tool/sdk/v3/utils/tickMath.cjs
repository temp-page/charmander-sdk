...
var _mostSignificantBit = require("./mostSignificantBit.cjs");
...
class TickMath {
  ...
  static getSqrtRatioAtTick(tick) {
    (0, _Common.invariant)(tick >= TickMath.MIN_TICK && tick <= TickMath.MAX_TICK && Number.isInteger(tick), "TICK");
    ...
    if (tick > 0) ratio = _constants.MaxUint256 / ratio;
    return ratio % Q32 > _internalConstants.ZERO ? ratio / Q32 + _internalConstants.ONE : ratio / Q32;
  }
  /**
  ...
  static getTickAtSqrtRatio(sqrtRatioX96) {
    (0, _Common.invariant)(sqrtRatioX96 >= TickMath.MIN_SQRT_RATIO && sqrtRatioX96 < TickMath.MAX_SQRT_RATIO, "SQRT_RATIO");
    const sqrtRatioX128 = sqrtRatioX96 << 32n;
    const msb = (0, _mostSignificantBit.mostSignificantBit)(sqrtRatioX128);
    let r;
    if (BigInt(msb) >= 128n) r = sqrtRatioX128 >> BigInt(msb - 127);else r = sqrtRatioX128 << BigInt(127 - msb);
