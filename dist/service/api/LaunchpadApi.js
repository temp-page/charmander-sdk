...
let LaunchpadApi = class LaunchpadApi {
    ...
    async staking(account = '') {
        ...
        const stakingPoolAbi = this.baseApi.connectInfo().create(abi_1.StakingPoolAbi);
        const [stakeInfos, batchGetTokens, [{ getScoreByTier, getUserScore, },],] = await Promise.all([
    ...
    async getTokenPriceHistory(token) {
        ...
        const { tokenDayDatas } = await this.baseApi.exchangeGraph(gql_1.QueryTokenATHPriceHistory, {
            token: token.toLowerCase(),
        });
        const tokenPrice = {
            ath: '0',
    ...
    async getUserDepositedLogsByPool(user, pool) {
        const res = await this.baseApi.launchpadGraph(gql_1.QueryIDOUserDepositedLogsByPool, {
            user: user.toLowerCase(),
    ...
    async poolDetail(pool, account = '') {
        ...
        const insurancePool = this.baseApi.connectInfo().create(abi_1.InsurancePoolAbi);
        ...
        depositInfo.claimLoss = (connect, insuranceId) => {
            return connect.create(abi_1.InsurancePoolAbi).claimLoss(insuranceId);
        };
