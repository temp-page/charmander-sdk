...
let PoolV3Api = PoolV3Api_1 = class PoolV3Api {
    ...
    async myLiquidityList(connectInfo) {
        const nonfungiblePositionManager = connectInfo.create(abi_1.NonfungiblePositionManager, this.baseApi.address().nonfungiblePositionManager);
    ...
    async positionHistoryByTokenId(tokenId) {
        const { positionSnapshots } = (await this.baseApi.exchangeGraph(gql_1.PositionHistoryGQL, { tokenId }));
    ...
    async myLiquidityByTokenId(connectInfo, tokenId) {
        ...
        liquidityInfo.collectFee = async (connect, involvesMNT) => {
            const transactionEvent = await connect.create(abi_1.NonfungiblePositionManager, this.baseApi.address().nonfungiblePositionManager)
    ...
    async feeTierDistribution(token0, token1) {
        const result = await this.baseApi.exchangeGraph(gql_1.FeeTierDistributionGQL, {
    ...
    async allTickInfo(token0, token1, feeAmount) {
        ...
        while (true) {
            const { ticks } = await this.baseApi.exchangeGraph(gql_1.AllV3TicksGQL, {
    ...
    async getPool(datas) {
        ...
        const poolDatas = await connectInfo.multiCall().call(...datas.map((data) => {
            ...
            const poolV3 = connectInfo.create(abi_1.PoolV3, address);
            return {
                address,
    ...
    async addLiquidity(token0, token1, account) {
        ...
        addLiquidityV3Info.addLiquidity = async (connect, allowedSlippage, deadline) => {
            const nonfungiblePositionManager = connect.create(abi_1.NonfungiblePositionManager);
