...
let DashboardApi = class DashboardApi {
    ...
    async getBlocksFromTimestamps(timestamps, type = 'exchange-v3') {
        if (timestamps?.length === 0)
            return [];
        const queryBlockMetaVo = await this.baseApi.exchangeGraph((0, gql_1.QueryBlockMeta)(), {});
        const maxBlockNumber = queryBlockMetaVo._meta.block.number;
        const timestampsResult = await this.baseApi.blockGraph((0, gql_1.QueryBlockTimeGQL)(timestamps), {});
        const blocks = [];
    ...
    async protocolData() {
        ...
        const globalResponses = await Promise.all(blockNumbers.map(it => this.baseApi.exchangeGraph((0, gql_1.globalDataGQL)(it), {})));
    ...
    async topPool() {
        ...
        const poolAddresses = (await this.baseApi.exchangeGraph(gql_1.topPoolsGQL, {})).pools.map(it => it.id);
        return await this.poolDatas(poolAddresses, block24, block48, block7d);
    }
    async ethPriceDatas() {
        const deltaTimestamps = tool_1.TimeUtils.getDeltaTimestamps();
        const [block24, block48, block7d] = await this.getBlocksFromTimestamps([deltaTimestamps.t24h, deltaTimestamps.t48h, deltaTimestamps.t7d]);
        const pricesResponse = (await this.baseApi.exchangeGraph((0, gql_1.ethPricesGQL)(block24.number, block48.number, block7d.number), {}));
        return {
    ...
    async topToken() {
        const deltaTimestamps = tool_1.TimeUtils.getDeltaTimestamps();
        const [block24, block48, block7d] = await this.getBlocksFromTimestamps([deltaTimestamps.t24h, deltaTimestamps.t48h, deltaTimestamps.t7d]);
        const tokenAddresses = (await this.baseApi.exchangeGraph(gql_1.topTokensGQL, {})).tokens
        ...
        for (const blockNumber of blockNumbers)
            tokenDataResponses.push(await this.baseApi.exchangeGraph((0, gql_1.tokensBulkGQL)(blockNumber, tokenAddresses), {}));
        const ethPrices = await this.ethPriceDatas();
    ...
    async poolDatas(poolAddresses, block24, block48, block7d) {
        if (poolAddresses.length === 0)
            return {};
        const blockNumbers = Array.from(new Set([undefined, block24.number, block48.number, block7d.number]));
        const poolDataResponses = await Promise.all(blockNumbers.map(it => this.baseApi.exchangeGraph((0, gql_1.poolsBulkGQL)(it, poolAddresses), {})));
