...
let DashboardApi = class DashboardApi {
    ...
    async protocolData() {
        ...
        const [block24] = await this.getBlocksFromTimestamps([deltaTimestamps.t24h]);
        const blockNumbers = Array.from(new Set([undefined, block24.number]));
        ...
        const data48 = globalResponses[blockNumbers.indexOf(block24.number)];
    ...
    async topPool() {
        ...
        return await this.poolDatas(poolAddresses, block24);
    }
    async ethPriceDatas() {
        const deltaTimestamps = tool_1.TimeUtils.getDeltaTimestamps();
        const [block24] = await this.getBlocksFromTimestamps([deltaTimestamps.t24h]);
        const pricesResponse = (await this.baseApi.exchangeV3Graph((0, DashboardGql_1.ethPricesGQL)(block24.number, block24.number, block24.number), {}));
        return {
    ...
    async topToken() {
        ...
        const data48 = tokenDataResponses[blockNumbers.indexOf(block24.number)];
    ...
    async poolDatas(poolAddresses, block24) {
        ...
        const data48 = poolDataResponses[blockNumbers.indexOf(block24.number)];
        const dataWeek = poolDataResponses[blockNumbers.indexOf(block24.number)];
