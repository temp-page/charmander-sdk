...
let SwapApi = class SwapApi {
    ...
    async swapInfo(token0, token1, account) {
        ...
        swapInfo.update = async () => {
            ...
            if (swapInfo.canSwap)
                await updateInput(swapInfo.intputToken, swapInfo.inputAmount, swapInfo.swapConfig, true);
        };
    ...
    async getTokenPrice(type, swapInfo) {
        ...
        if (type === 'month' || type === 'year' || type === 'day' || type === 'week') {
            ...
            const price24 = price24Token0 && price24Token1 ? new bignumber_js_1.default(price24Token0).div(price24Token1).toFixed() : '0';
            ...
            const lastPrice = token1PriceData[token1PriceData.length - 1]?.price || '0';
            const change = new bignumber_js_1.default(lastPrice).minus(price24).toFixed();
            swapInfo.tokenPrice = {
                datas: token1PriceData,
                lastPrice: token1PriceData[token1PriceData.length - 1]?.price || '0',
                change24h: parseFloat(price24) <= 0 ? '0' : new bignumber_js_1.default(lastPrice).minus(price24).div(price24).multipliedBy(100).toFixed(2),
                change: change,
            };
        }
    }
