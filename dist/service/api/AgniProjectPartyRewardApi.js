...
let AgniProjectPartyRewardApi = class AgniProjectPartyRewardApi {
    ...
    async result(account) {
        ...
        if (account) {
            let userPools = partyReward.pools.filter(it => (0, tool_1.eqAddress)(it.userAddress, account));
            let dashboardPoolDataResponse = [];
            if (userPools.length > 0) {
                dashboardPoolDataResponse = (await this.baseApi.exchangeGraph((0, gql_1.poolsBulkGQL)(undefined, userPools.map(it => it.poolAddress.toLowerCase())), {})).pools;
            }
            let dashboardPoolDataMap = (0, groupBy_1.default)(dashboardPoolDataResponse, it => it.id);
            ...
            let [[{ getReward }], { claimLogs }] = await Promise.all([
            ...
            projectPartyRewardMyListResult.pools = userPools.map((it, index) => {
                let dashboardPoolDataMapElement = dashboardPoolDataMap[it.poolAddress.toLowerCase()] || [];
