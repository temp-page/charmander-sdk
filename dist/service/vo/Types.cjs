...
class Balance {
  ...
  select(rate) {
    let selectRate = rate;
    if (rate === "max") selectRate = "100";
    return new _bignumber.default(this.balance).multipliedBy(selectRate).div("100").toFixed();
  }
}
...
class BalanceAndAllowance extends Balance {
  ...
  showApprove(inputAmount) {
    if (this.token.address === ETH_ADDRESS || this.spender === "") return false;
    return new _bignumber.default(inputAmount).comparedTo(this.allowance) > 0;
  }
  approve(connectInfo) {
