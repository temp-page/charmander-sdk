...
function getValue(obj, path, defaultValue) {
  return lodashEs.get(obj, path, defaultValue) || defaultValue;
}
        ...
        return function() {
          ...
          try {
            if (ins.methodCache) {
              ...
              const compiled = lodashEs.template(ins.methodCacheKey);
...
class Token extends BaseCurrency {
   ...
   */
  equals(other) {
    return !isNullOrUndefined(other) && other.isToken && this.chainId === other.chainId && this.address === other.address;
  }
  /**
...
async function multicallExecute(multicall, calls) {
  ...
  const callRequestsChuck = lodashEs.chunk(callRequests, CHUNK_SIZE);
  try {
    const response = [];
...
exports.ERC20 = __decorateClass$g([
  CacheKey("ERC20")
], exports.ERC20);
...
exports.MultiCallContract = class MultiCallContract extends BaseAbi {
  ...
  async call(...shapeWithLabels) {
    ...
    shapeWithLabels.forEach((relay) => {
      const pairs = lodashEs.toPairs(relay);
      pairs.forEach(([, value]) => {
...
exports.MultiCallContract = __decorateClass$f([
...
exports.PoolV3 = __decorateClass$e([
  CacheKey("PoolV3")
], exports.PoolV3);
...
exports.NonfungiblePositionManager = __decorateClass$d([
...
exports.StakingPoolAbi = __decorateClass$c([
...
exports.InsurancePoolAbi = __decorateClass$b([
...
exports.IdoPoolAbi = __decorateClass$a([
  CacheKey("IdoPoolAbi")
], exports.IdoPoolAbi);
...
exports.IQuoterV2Abi = __decorateClass$9([
...
exports.GasMultiCallContract = __decorateClass$8([
...
exports.PoolV3Api = __decorateClass$7([
  CacheKey("PoolV3Api")
], exports.PoolV3Api);
...
function getPairCombinations(currencyA, currencyB) {
  ...
  const basePairs = lodashEs.flatMap(bases, (base) => bases.map((otherBase) => [base, otherBase]));
...
function getBestSwapRouteBy(tradeType, percentToQuotes, percents, chainId, by, { maxSplits = 4, minSplits = 0 }) {
  const percentToSortedQuotes = lodashEs.mapValues(percentToQuotes, (routeQuotes) => {
  ...
  if (!percentToSortedQuotes[100] || minSplits > 1) {
    console.log(
      {
        percentToSortedQuotes: lodashEs.mapValues(percentToSortedQuotes, (p) => p.length)
      },
...
function findFirstRouteNotUsingUsedPools(usedRoutes, candidateRouteQuotes) {
  ...
  const usedPoolAddresses = lodashEs.flatMap(usedRoutes, ({ pools }) => pools.map((it) => it.address));
...
async function createGasModel({
  ...
  const estimateGasCost = (pools, initializedTickCrossedList) => {
    const totalInitializedTicksCrossed = BigInt(Math.max(1, lodashEs.sum(initializedTickCrossedList)));
    let baseGasUse = 0n;
    baseGasUse += BASE_SWAP_COST_V3(chainId);
    lodashEs.forEach(pools, () => {
...
exports.SwapV3Api = class SwapV3Api {
  ...
  async getCandidatePoolsOnGraphNode(tokenA, tokenB, mataData) {
    const metaDataGroup = lodashEs.groupBy(mataData, (it) => it.address.toLowerCase());
  ...
  async getBestTrade(amount, token0, token1, inputToken) {
    ...
    const routerTrade = await getBestTrade(
      ...
      tradeType,
      {
        ...
        poolProvider: this.getPoolProvider(),
...
exports.SwapV3Api = __decorateClass$6([
  CacheKey("SwapV3Api")
], exports.SwapV3Api);
...
exports.TokenMangerApi = class TokenMangerApi {
  ...
  async batchGetTokens(addresses) {
    const tokenAddressMap = lodashEs.groupBy(await this.getTokenByTokenList(), (it) => it.address);
  ...
  async tokenPrice(...tokens2) {
    ...
    const _groupBy = lodashEs.groupBy(tokenPrices, (it) => it.id.toLowerCase());
    return tokens2.map((token) => {
      const mntPrice = lodashEs.get(bundles, "0.ethPriceUSD", "0");
  ...
  async tokenList(url = "") {
    if (url === "") {
      ...
      return [
        this.mapToTokenList({
          url,
          enable: lodashEs.get(storageTokenListInfo, "enable", false)
  ...
  async searchTokenList(urls) {
    for (let i = 0; i < urls.length; i++) {
      ...
      if (json.tokens) {
        lodashEs.remove(json.tokens, (token) => {
...
exports.TokenMangerApi = __decorateClass$5([
...
let LaunchpadApi = class {
  ...
  async getTokenPriceHistory(token) {
    ...
    tokenPrice.last = lodashEs.get(tokenDayDatas, "[0].priceUSD", "0");
  ...
  async getPools() {
    ...
    idoPoolStatistics.totalParticipants = lodashEs.get(res, "idoPoolStatistics.totalParticipants", "0");
  ...
  async poolDetail(pool, account = "") {
    ...
    if (depositInfo.claimStatus === "enable") {
      if (new BigNumber__default(depositInfo.avgPrice).comparedTo("0") > 0 && new BigNumber__default(depositInfo.needToPay).comparedTo("0") > 0) {
        ...
        const time = lodashEs.get(launchpadInfo, "redemption_time", "0");
        let state = "disabled";
    ...
    poolDetail.publicAllocation = BigNumber__default.max(new BigNumber__default(lodashEs.get(launchpadInfo, "pool_size", "0")).minus(poolInfo.whiteListQuota), "0").toFixed();
    ...
    if (launchpadInfo) {
      poolDetail.poolSize = lodashEs.get(launchpadInfo, "pool_size", "");
      poolDetail.initialMarketCap = lodashEs.get(launchpadInfo, "initial_market_cap", "");
      poolDetail.FDV = lodashEs.get(launchpadInfo, "fdv", "");
      poolDetail.tags = lodashEs.get(launchpadInfo, "selling_token_tag", "");
      poolDetail.whitelistStakingTierRequired = lodashEs.get(launchpadInfo, "whitelist_staking_tier_required", "");
      poolDetail.whitelistRegistrationRequired = lodashEs.get(launchpadInfo, "whitelist_registration_required", "");
      poolDetail.whitelistDistribution = lodashEs.get(launchpadInfo, "whitelist_distribution", "");
      poolDetail.publicStakingTierRequired = lodashEs.get(launchpadInfo, "public_registration_required", "");
      poolDetail.publicRegistrationRequired = lodashEs.get(launchpadInfo, "public_registration_required", "");
      poolDetail.publicDistribution = lodashEs.get(launchpadInfo, "public_distribution", "");
      poolDetail.introduction = lodashEs.get(launchpadInfo, "introduction", "");
      poolDetail.shares = lodashEs.get(launchpadInfo, "shares", []).filter((it) => it.icon);
    } else {
      poolDetail.poolSize = "";
...
exports.DashboardApi = class DashboardApi {
  ...
  async protocolData() {
    ...
    const volumeOneWindowAgo = Number.parseFloat(lodashEs.get(parsed24, "totalVolumeUSD", "0")) - Number.parseFloat(lodashEs.get(parsed48, "totalVolumeUSD", "0"));
    ...
    const txCountOneWindowAgo = Number.parseFloat(lodashEs.get(parsed24, "txCount", "0")) - Number.parseFloat(lodashEs.get(parsed48, "txCount", "0"));
    ...
    const feesOneWindowAgo = new BigNumber__default(lodashEs.get(parsed24, "totalFeesUSD", "0")).minus(lodashEs.get(parsed24, "totalProtocolFeesUSD", "0")).minus(new BigNumber__default(lodashEs.get(parsed48, "totalFeesUSD", "0")).minus(lodashEs.get(parsed48, "totalProtocolFeesUSD", "0")));
  ...
  async ethPriceDatas() {
    ...
    return {
      current: Number.parseFloat(lodashEs.get(pricesResponse, "current[0].ethPriceUSD", "0")),
      oneDay: Number.parseFloat(lodashEs.get(pricesResponse, "oneDay[0].ethPriceUSD", "0")),
      twoDay: Number.parseFloat(lodashEs.get(pricesResponse, "twoDay[0].ethPriceUSD", "0")),
      week: Number.parseFloat(lodashEs.get(pricesResponse, "oneWeek[0].ethPriceUSD", "0"))
    };
  }
  async topToken() {
    const deltaTimestamps = TimeUtils.getDeltaTimestamps();
...
exports.DashboardApi = __decorateClass$3([
...
exports.TransactionService = class TransactionService extends BaseService {
  ...
  convertErrorMsg(e) {
    ...
    let recursiveErr = e;
    let reason;
    if (lodashEs.get(recursiveErr, "data.message")) {
      reason = lodashEs.get(recursiveErr, "data.message");
    } else {
      while (recursiveErr) {
        reason = lodashEs.get(recursiveErr, "reason") || lodashEs.get(recursiveErr, "data.message") || lodashEs.get(recursiveErr, "message") || lodashEs.get(recursiveErr, "info.error.message") || reason;
        recursiveErr = lodashEs.get(recursiveErr, "error") || lodashEs.get(recursiveErr, "data.originalError") || lodashEs.get(recursiveErr, "info");
      }
    }
...
class WalletConnect {
  ...
  static getEthereum() {
    return lodashEs.get(window, "ethereum");
  }
  /**
   * 链接钱包
   * @returns
