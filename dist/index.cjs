...
const FixedReverseHeap = require('mnemonist/fixed-reverse-heap.js');
const Queue = require('mnemonist/queue.js');
...
const fromPairs__default = /*#__PURE__*/_interopDefaultCompat(fromPairs);
const toPairs__default = /*#__PURE__*/_interopDefaultCompat(toPairs);
const chunk__default = /*#__PURE__*/_interopDefaultCompat(chunk);
const axios__default = /*#__PURE__*/_interopDefaultCompat(axios);
const groupBy__default = /*#__PURE__*/_interopDefaultCompat(groupBy);
const flatMap__default = /*#__PURE__*/_interopDefaultCompat(flatMap);
const forEach__default = /*#__PURE__*/_interopDefaultCompat(forEach);
const sum__default = /*#__PURE__*/_interopDefaultCompat(sum);
const FixedReverseHeap__default = /*#__PURE__*/_interopDefaultCompat(FixedReverseHeap);
...
const remove__default = /*#__PURE__*/_interopDefaultCompat(remove);
...
function getValue(obj, path, defaultValue) {
  return get__default(obj, path, defaultValue) || defaultValue;
}
        ...
        return function() {
          ...
          try {
            if (ins.methodCache) {
              ...
              const compiled = template__default(ins.methodCacheKey);
...
async function multicallExecute(multicall, calls) {
  ...
  const callRequestsChuck = chunk__default(callRequests, CHUNK_SIZE);
  try {
    const response = [];
...
exports.MultiCallContract = class MultiCallContract extends BaseAbi {
  ...
  async call(...shapeWithLabels) {
    ...
    shapeWithLabels.forEach((relay) => {
      const pairs = toPairs__default(relay);
      pairs.forEach(([, value]) => {
    ...
    const datas = shapeWithLabels.map((relay) => {
      ...
      return fromPairs__default(pairs);
    });
    return datas;
  }
};
...
function getPairCombinations(currencyA, currencyB) {
  ...
  const basePairs = flatMap__default(bases, (base) => bases.map((otherBase) => [base, otherBase]));
...
function getBestSwapRouteBy(tradeType, percentToQuotes, percents, chainId, by, { maxSplits = 4, minSplits = 0 }) {
  const percentToSortedQuotes = mapValues__default(percentToQuotes, (routeQuotes) => {
  ...
  let bestQuote;
  let bestSwap;
  const bestSwapsPerSplit = new FixedReverseHeap__default(
    Array,
    (a, b) => {
  ...
  if (!percentToSortedQuotes[100] || minSplits > 1) {
    console.log(
      {
        percentToSortedQuotes: mapValues__default(percentToSortedQuotes, (p) => p.length)
      },
  ...
  const queue = new Queue__default();
...
function findFirstRouteNotUsingUsedPools(usedRoutes, candidateRouteQuotes) {
  ...
  const usedPoolAddresses = flatMap__default(usedRoutes, ({ pools }) => pools.map((it) => it.address));
...
async function createGasModel({
  ...
  const estimateGasCost = (pools, initializedTickCrossedList) => {
    const totalInitializedTicksCrossed = BigInt(Math.max(1, sum__default(initializedTickCrossedList)));
    let baseGasUse = 0n;
    baseGasUse += BASE_SWAP_COST_V3(chainId);
    forEach__default(pools, () => {
...
exports.SwapV3Api = class SwapV3Api {
  ...
  async getCandidatePoolsOnGraphNode(tokenA, tokenB, mataData) {
    const metaDataGroup = groupBy__default(mataData, (it) => it.address.toLowerCase());
...
exports.TokenMangerApi = class TokenMangerApi {
  ...
  async batchGetTokens(addresses) {
    const tokenAddressMap = groupBy__default(await this.getTokenByTokenList(), (it) => it.address);
  ...
  async tokenPrice(...tokens2) {
    ...
    const _groupBy = groupBy__default(tokenPrices, (it) => it.id.toLowerCase());
    return tokens2.map((token) => {
      const mntPrice = get__default(bundles, "0.ethPriceUSD", "0");
  ...
  async tokenList(url = "") {
    if (url === "") {
      ...
      return [
        this.mapToTokenList({
          url,
          enable: get__default(storageTokenListInfo, "enable", false)
  ...
  async searchTokenList(urls) {
    for (let i = 0; i < urls.length; i++) {
      ...
      if (json.tokens) {
        remove__default(json.tokens, (token) => {
...
let LaunchpadApi = class {
  ...
  async getTokenPriceHistory(token) {
    ...
    tokenPrice.last = get__default(tokenDayDatas, "[0].priceUSD", "0");
  ...
  async getPools() {
    ...
    idoPoolStatistics.totalParticipants = get__default(res, "idoPoolStatistics.totalParticipants", "0");
  ...
  async poolDetail(pool, account = "") {
    ...
    if (depositInfo.claimStatus === "enable") {
      if (new BigNumber__default(depositInfo.avgPrice).comparedTo("0") > 0 && new BigNumber__default(depositInfo.needToPay).comparedTo("0") > 0) {
        ...
        const time = get__default(launchpadInfo, "redemption_time", "0");
        let state = "disabled";
    ...
    poolDetail.publicAllocation = BigNumber__default.max(new BigNumber__default(get__default(launchpadInfo, "pool_size", "0")).minus(poolInfo.whiteListQuota), "0").toFixed();
    ...
    if (launchpadInfo) {
      poolDetail.poolSize = get__default(launchpadInfo, "pool_size", "");
      poolDetail.initialMarketCap = get__default(launchpadInfo, "initial_market_cap", "");
      poolDetail.FDV = get__default(launchpadInfo, "fdv", "");
      poolDetail.tags = get__default(launchpadInfo, "selling_token_tag", "");
      poolDetail.whitelistStakingTierRequired = get__default(launchpadInfo, "whitelist_staking_tier_required", "");
      poolDetail.whitelistRegistrationRequired = get__default(launchpadInfo, "whitelist_registration_required", "");
      poolDetail.whitelistDistribution = get__default(launchpadInfo, "whitelist_distribution", "");
      poolDetail.publicStakingTierRequired = get__default(launchpadInfo, "public_registration_required", "");
      poolDetail.publicRegistrationRequired = get__default(launchpadInfo, "public_registration_required", "");
      poolDetail.publicDistribution = get__default(launchpadInfo, "public_distribution", "");
      poolDetail.introduction = get__default(launchpadInfo, "introduction", "");
      poolDetail.shares = get__default(launchpadInfo, "shares", []).filter((it) => it.icon);
    } else {
      poolDetail.poolSize = "";
...
exports.DashboardApi = class DashboardApi {
  ...
  async protocolData() {
    ...
    const volumeOneWindowAgo = Number.parseFloat(get__default(parsed24, "totalVolumeUSD", "0")) - Number.parseFloat(get__default(parsed48, "totalVolumeUSD", "0"));
    ...
    const txCountOneWindowAgo = Number.parseFloat(get__default(parsed24, "txCount", "0")) - Number.parseFloat(get__default(parsed48, "txCount", "0"));
    ...
    const feesOneWindowAgo = new BigNumber__default(get__default(parsed24, "totalFeesUSD", "0")).minus(get__default(parsed24, "totalProtocolFeesUSD", "0")).minus(new BigNumber__default(get__default(parsed48, "totalFeesUSD", "0")).minus(get__default(parsed48, "totalProtocolFeesUSD", "0")));
  ...
  async ethPriceDatas() {
    ...
    return {
      current: Number.parseFloat(get__default(pricesResponse, "current[0].ethPriceUSD", "0")),
      oneDay: Number.parseFloat(get__default(pricesResponse, "oneDay[0].ethPriceUSD", "0")),
      twoDay: Number.parseFloat(get__default(pricesResponse, "twoDay[0].ethPriceUSD", "0")),
      week: Number.parseFloat(get__default(pricesResponse, "oneWeek[0].ethPriceUSD", "0"))
    };
  }
  async topToken() {
    const deltaTimestamps = TimeUtils.getDeltaTimestamps();
  ...
  async result(account) {
    ...
    if (account) {
      ...
      projectPartyRewardMyListResult.histories = getReward.infos.map((it) => {
        const hash = get__default(claimLogs.find((claim) => claim.epoch === it.epoch), "hash", "");
        return {
          claim: it.claim,
          epoch: it.epoch,
          amount: it.amount,
...
exports.TransactionService = class TransactionService extends BaseService {
  ...
  convertErrorMsg(e) {
    ...
    let recursiveErr = e;
    let reason;
    if (get__default(recursiveErr, "data.message")) {
      reason = get__default(recursiveErr, "data.message");
    } else {
      while (recursiveErr) {
        reason = get__default(recursiveErr, "reason") || get__default(recursiveErr, "data.message") || get__default(recursiveErr, "message") || get__default(recursiveErr, "info.error.message") || reason;
        recursiveErr = get__default(recursiveErr, "error") || get__default(recursiveErr, "data.originalError") || get__default(recursiveErr, "info");
      }
    }
...
class WalletConnect {
  ...
  static getEthereum() {
    return get__default(window, "ethereum");
  }
  /**
   * 链接钱包
   * @returns ConnectInfo
