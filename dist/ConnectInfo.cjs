...
class ConnectInfo {
  ...
  create(clazz, ...args) {
    ...
    if (!cacheKey) return new clazz(this, ...args);
    const key = `${cacheKey}_${JSON.stringify(args)}`;
    const element = this._instanceCache.get(key);
    if (typeof element !== "undefined") {
      return element;
    } else {
      const instance = (0, _service.createProxy)(new clazz(this, ...args));
  ...
  clear() {
    ...
    (0, _service.clearCache)();
  }
  /**
   * 获取 ERC20 API
   */
  erc20() {
    return this.create(_service.Erc20Service);
  }
  /**
   * 获取交易API
   */
  tx() {
    return this.create(_service.TransactionService);
  }
  /**
   * multiCall service
   */
  multiCall() {
    return this.create(_service.MultiCallContract);
  }
  get provider() {
    if (this._status) return this._provider;
    throw new _BasicException.BasicException("Wallet not connected!");
  }
  set provider(value) {
  ...
  async addToken(tokenAddress) {
    ...
    _service.Trace.debug("token info", token);
    try {
