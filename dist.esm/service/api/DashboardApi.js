...
import { QueryBlockMeta, QueryBlockTimeGQL, ethPricesGQL, globalChartGQL, globalDataGQL, globalTransactionsGQL, poolsBulkGQL, tokensBulkGQL, topPoolsGQL, topTokensGQL, } from './gql';
let DashboardApi = class DashboardApi {
    ...
    async getBlocksFromTimestamps(timestamps, type = 'exchange-v3') {
        ...
        const timestampsResult = await this.baseApi.blockGraph(QueryBlockTimeGQL(timestamps), {});
        const blocks = [];
    ...
    async protocolData() {
        ...
        const blockNumbers = Array.from(new Set([undefined, block24.number, block48.number]));
        const globalResponses = await Promise.all(blockNumbers.map(it => this.baseApi.exchangeGraph(globalDataGQL(it), {})));
    ...
    async topPool() {
        const deltaTimestamps = TimeUtils.getDeltaTimestamps();
        const [block24, block48, block7d] = await this.getBlocksFromTimestamps([deltaTimestamps.t24h, deltaTimestamps.t48h, deltaTimestamps.t7d]);
    ...
    async ethPriceDatas() {
        const deltaTimestamps = TimeUtils.getDeltaTimestamps();
        const [block24, block48, block7d] = await this.getBlocksFromTimestamps([deltaTimestamps.t24h, deltaTimestamps.t48h, deltaTimestamps.t7d]);
        const pricesResponse = (await this.baseApi.exchangeGraph(ethPricesGQL(block24.number, block48.number, block7d.number), {}));
        return {
    ...
    async topToken() {
        const deltaTimestamps = TimeUtils.getDeltaTimestamps();
        const [block24, block48, block7d] = await this.getBlocksFromTimestamps([deltaTimestamps.t24h, deltaTimestamps.t48h, deltaTimestamps.t7d]);
        const tokenAddresses = (await this.baseApi.exchangeGraph(topTokensGQL, {})).tokens
        ...
        if (tokenAddresses.length === 0)
            return {};
        const blockNumbers = Array.from(new Set([undefined, block24.number, block48.number, block7d.number]));
        ...
        const formatted = tokenAddresses.reduce((accum, address) => {
            ...
            const tvlUSDChange = DashboardMath.getPercentChange(Number.parseFloat(current?.totalValueLockedUSD).toFixed(), Number.parseFloat(oneDay?.totalValueLockedUSD).toFixed());
    ...
    async poolDatas(poolAddresses, block24, block48, block7d) {
        if (poolAddresses.length === 0)
            return {};
        const blockNumbers = Array.from(new Set([undefined, block24.number, block48.number, block7d.number]));
        const poolDataResponses = await Promise.all(blockNumbers.map(it => this.baseApi.exchangeGraph(poolsBulkGQL(it, poolAddresses), {})));
        ...
        const formatted = poolAddresses.reduce((accum, address) => {
            ...
            const tvlUpdated = current
                ? new BigNumber(tvlToken0).multipliedBy(current.token0.derivedETH).multipliedBy(ethPriceUSD)
                    .plus(new BigNumber(tvlToken1).multipliedBy(current.token1.derivedETH).multipliedBy(ethPriceUSD)).toFixed()
                : undefined;
            if (tvlUpdated)
                tvlUSD = tvlUpdated;
