...
import { AddLiquidityV3Info, BalanceAndAllowance, ETH_ADDRESS, FeeAmount, LiquidityInfo, LiquidityListData, TICK_SPACINGS, } from '../vo';
import { NonfungiblePositionManager, PoolV3 } from '../abi';
...
import { AllV3TicksGQL, FeeTierDistributionGQL, PositionHistoryGQL } from './gql';
...
let PoolV3Api = PoolV3Api_1 = class PoolV3Api {
    ...
    async myLiquidityList(connectInfo) {
        ...
        const batchGetTokens = await this.baseApi.address().getApi().tokenMangerApi().batchGetTokens(Array.from(new Set(positions.map(it => [it.position.token0, it.position.token1, ETH_ADDRESS]).flatMap(it => it))));
    ...
    async myLiquidityByTokenId(connectInfo, tokenId) {
        ...
        const batchGetTokens = await this.baseApi.address().getApi().tokenMangerApi().batchGetTokens(Array.from(new Set([position.token0, position.token1, ETH_ADDRESS])));
        ...
        liquidityInfo.collectFee = async (connect, involvesMNT) => {
            const transactionEvent = await connect.create(NonfungiblePositionManager, this.baseApi.address().nonfungiblePositionManager)
        ...
        liquidityInfo.removeLiquidity = async (connect, rate, involvesMNT, allowedSlippage, deadline) => {
            ...
            const transactionEvent = await connect.create(NonfungiblePositionManager, this.baseApi.address().nonfungiblePositionManager)
                .removeLiquidity(rate, token0, token1, positionDetail, tokenId, collect.amount0, collect.amount1, involvesMNT, new Percent(BigInt(new BigNumber(allowedSlippage).multipliedBy(10000).toFixed(0, BigNumber.ROUND_DOWN)), 10000n), Number.parseInt(String(new Date().getTime() / 1000)) + Number.parseInt(deadline.toString()));
            transactionHistory.saveHistory(connectInfo, transactionEvent, {
                token0,
                token1,
    ...
    static computePoolAddress(tokenA, tokenB, fee) {
        ...
        const encodedParams = AbiCoder.defaultAbiCoder().encode(['address', 'address', 'uint24'], [token0.erc20Address(), token1.erc20Address(), fee]);
        const salt = solidityPackedKeccak256(['bytes'], [encodedParams]);
        return getCreate2Address(agniPoolDeployer, salt, poolInitCodeHash);
    }
    async getPool(datas) {
    ...
    parsePrice(token0, token1, inputFirstPrice) {
        ...
        return new Price(baseAmount.currency, parsedQuoteAmount.currency, baseAmount.quotient, parsedQuoteAmount.quotient);
    }
    // 获取输出币种金额
    ...
    async addLiquidity(token0, token1, account) {
        ...
        addLiquidityV3Info.addLiquidity = async (connect, allowedSlippage, deadline) => {
            ...
            transactionHistory.saveHistory(connect, transactionEvent, {
                token0,
                token1,
...
PoolV3Api = PoolV3Api_1 = __decorate([
    CacheKey('PoolV3Api'),
