...
import { IDODepositInfo, IDOPool, IDOPoolDetail, IDOUserDepositInfo, IdoPoolStatistic, LaunchpadStakeDetail, } from '../vo';
import { ERC20, IdoPoolAbi, InsurancePoolAbi, StakingPoolAbi } from '../abi';
import { BasicException } from '../../BasicException';
import { CacheKey, INVALID_ADDRESS, Trace, isNumber } from '../tool';
import { QueryIDOPoolInfo, QueryIDOUserDepositedLogsByPool, QueryIdoPoolInfosGQL, QueryTokenATHPriceHistory, UserStakeInfosGQL, } from './gql';
...
let LaunchpadApi = class LaunchpadApi {
    ...
    async getUserStakeInfoByGql(user, token) {
        const res = await this.baseApi.launchpadGraph(UserStakeInfosGQL, {
    ...
    async getTokenPriceHistory(token) {
        ...
        const { tokenDayDatas } = await this.baseApi.exchangeGraph(QueryTokenATHPriceHistory, {
            token: token.toLowerCase(),
        });
        const tokenPrice = {
            ath: '0',
    ...
    async getUserDepositedLogsByPool(user, pool) {
        const res = await this.baseApi.launchpadGraph(QueryIDOUserDepositedLogsByPool, {
            user: user.toLowerCase(),
    ...
    async poolDetail(pool, account = '') {
        ...
        depositInfo.maxRaisingAmount = new BigNumber(poolInfo.publicQuota).multipliedBy(poolInfo.publicSalePrice).plus(new BigNumber(poolInfo.whiteListQuota).multipliedBy(poolInfo.presalePrice)).toFixed();
        ...
        depositInfo.deposit = async (connect, buyInsurance, depositAmount, extraDeposit) => {
            ...
            if (depositInfo.currentDeposit.type === 'whiteList') {
                ...
                return connect.create(IdoPoolAbi, pool).presaleDeposit(buyQuota, buyInsurance);
            }
        ...
        if (depositInfo.claimStatus === 'enable') {
            if (new BigNumber(depositInfo.avgPrice).comparedTo('0') > 0 && new BigNumber(depositInfo.needToPay).comparedTo('0') > 0) {
                ...
                const handleInsuranceDetails = (depositData, insuranceDetail, insuranceId) => {
                    ...
                    const payAmount = new BigNumber(result.buyQuota.toString()).div(sellingTokenDiv).multipliedBy(new BigNumber(result.price.toString()).div(raisingTokenDiv).minus(depositInfo.avgPrice)).toFixed();
