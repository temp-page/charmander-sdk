...
export function getBestRouteCombinationByQuotes(amount, quoteCurrency, routesWithQuote, tradeType, config) {
    ...
    const totalAmount = routeAmounts.reduce((total, routeAmount) => total.add(routeAmount.amount), CurrencyAmount.fromRawAmount(routeAmounts[0].amount.currency, 0));
...
export function getBestSwapRouteBy(tradeType, percentToQuotes, percents, chainId, by, { maxSplits = 4, minSplits = 0 }) {
    ...
    const quoteCompFn = tradeType === TradeType.EXACT_INPUT
        ? (a, b) => a.greaterThan(b)
        : (a, b) => a.lessThan(b);
        ...
        while (layer > 0) {
            ...
            for (let i = percentIndex; i >= 0; i--) {
                ...
                if (remainingPercentNew === 0 && splits >= minSplits) {
                    ...
                    const quoteAfterL1Adjust = tradeType === TradeType.EXACT_INPUT
